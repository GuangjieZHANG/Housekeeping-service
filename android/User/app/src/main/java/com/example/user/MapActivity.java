...
import baiduTool.overlayutil.PoiOverlay;
...
public class MapActivity extends Activity {
  ...
  //  boolean isFirstLoc = true;// 是否首次定位
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        session=its.getStringExtra("session");

        submit=(Button)findViewById(R.id.submit_map);

        submit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //页面传值  把经纬度传到订单页面  然后跳转
                Intent itgoordering=new Intent(MapActivity.this,OrdingActivity.class);
               // String jingdu=Double.toString(longtitude);
              // String weidu=Double.toString(latitude);
                itgoordering.putExtra("session",session);
               itgoordering.putExtra("jingdu",longtitude);
                itgoordering.putExtra("weidu",latitude);
                itgoordering.putExtra("add",add);
                MapActivity.this.startActivity(itgoordering);
            }
        });
        mapView = (MapView) this.findViewById(R.id.ordermap); // 获取地图控件引用
        baiduMap = mapView.getMap(); //报空指针错误
        ///这里没有错误  小傻子把地图id搞错了~~~冷汗
        //开启定位图层
        ...
        public void onReceiveLocation(BDLocation loc) {
            if (loc != null && (loc.getLocType() == 161 || loc.getLocType() == 66)) {
                ...
                if (locData != null) {
                    ...
                    if (!toastLoad) {
                        Toast.makeText(MapActivity.this, "正在加载地图...", Toast.LENGTH_SHORT).show();
                    }
                    toastLoad = true;
                    locationClient.stop();
                }
            } else {
                Toast.makeText(MapActivity.this, "定位失败，请检查手机网络或设置！", Toast.LENGTH_LONG).show();
            }
        }
        ...
        public void handleMessage(Message msg) {
            ...
            try {
                ...
                if (location.getLocType() == BDLocation.TypeGpsLocation) {
                    ...
                    Toast.makeText(MapActivity.this, "服务端网络定位失败，可以反馈IMEI号和大体定位时间到loc-bugs@baidu.com，会有人追查原因", Toast.LENGTH_LONG).show();
                } else if (location.getLocType() == BDLocation.TypeNetWorkException) {
                    Toast.makeText(MapActivity.this, "网络不同导致定位失败，请检查网络是否通畅", Toast.LENGTH_LONG).show();
                } else if (location.getLocType() == BDLocation.TypeCriteriaException) {
                    Toast.makeText(MapActivity.this, "无法获取有效定位依据导致定位失败，一般是由于手机的原因，处于飞行模式下一般会造成这种结果，可以试着重启手机", Toast.LENGTH_LONG).show();
                }
