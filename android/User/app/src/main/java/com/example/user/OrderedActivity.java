...
import com.example.alipay.AliPay;
import com.example.alipay.AuthResult;
...
//        orderedfinishtime.setText("订单结束时间："+orded.getFinishTime().toString());
        ...
        gopay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //去付款
               AliPay.Builder builder=new AliPay.Builder(OrderedActivity.this);
                builder.setSELLER("stkdfh7303@sandbox.com")//设置卖方账户 此处使用沙箱测试
                .setPARTNER("2088102169779413")
                .setRSA_PRIVATE(rsa_private)
                .setNotifyURL("https://openapi.alipaydev.com/gateway.do")
                .setOrderTitle("阿姨来了订单")
                .setSubTitle("测试1")
                .setPrice("0.01")
                .setPayCallBackListener(new AliPay.Builder.PayCallBackListener() {
                    ...
                    public void onPayCallBack(int status, String resultStatus, String progress) {
                       //支付回调
                        toast(progress);
                        Log.d("debug",resultStatus+":"+progress);
                    }
                });
                builder.pay();
