...
public class UnorderAction extends BaseAction{
	...
	public String getUnorderList(){
		...
		return this.ajax(List2json(pm.getList()), "application/json");
	}

	@Action("/user_unorders")
	public String getUserUnorders(){
		User user = userService.getUserById((String)session.get("uid"));
//		User user = userService.getUserById("8a00b3825a513224015a5132856b0000");
		List<Unorder> unorders = new ArrayList<Unorder>(user.getUnorders());
		return this.ajax(List2json(unorders), "application/json");
	}
	
	@Action("/worker_unorders")
	public String getWorkerUnorders(){
		Worker worker = workerService.getWorkerById((String)session.get("wid"));
		List<Unorder> unorders = new ArrayList<Unorder>(worker.getUnorders());
		return this.ajax(List2json(unorders), "application/json");
	}
	...
	@Action("/Unorder_Save")
	public String save(){
		System.out.println(unorderJson);
		System.out.println(editorType);
		unorder = BaseAction.Json2Object(unorderJson, Unorder.class);
		System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>");
		System.out.println(unorder.getOrderId());
		System.out.println(unorder.getAddress());
		System.err.println(unorder.getUser().getUserName());
		System.out.println(unorder.getServicetype().getDescription());
		System.out.println("<<<<<<<<<<<<<<<<<<<<<<<<<<");
	...
	/**
	 * @return the unorderJson
	 */
	public String getUnorderJson() {
		return unorderJson;
	}

	/**
	 * @param unorderJson the unorderJson to set
	 */
	public void setUnorderJson(String unorderJson) {
		this.unorderJson = unorderJson;
	}
