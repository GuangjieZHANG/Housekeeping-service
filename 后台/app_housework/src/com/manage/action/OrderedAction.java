...
public class OrderedAction extends BaseAction{
	...
	@Action("/user_ordereds")
	public String getUserOrdereds(){
		User user = userService.getUserById((String)session.get("uid"));
		List<Ordered> ordereds = new ArrayList<Ordered>(user.getOrdereds());
		return this.ajax(List2json(ordereds), "application/json");
	}
	
	@Action("/worker_ordereds")
	public String getWorkerOrdereds(){
		Worker worker = workerService.getWorkerById((String)session.get("wid"));
		List<Ordered> ordereds = new ArrayList<Ordered>(worker.getOrdereds());
		return this.ajax(List2json(ordereds), "application/json");
	}
